...
const SearchSuppliers = (props) => {
    ...
    useEffect(() => {
        Axios.get('http://localhost:8090/supplier/activesuppliers/').then(res => {
            ...
            setSelectedSuppliers(res.data.filter((supplier) => supplier.name.toLowerCase().includes(id.toLowerCase())));
...
{(selectedSuppliers.length === 0 ) ? (
          <div >
            ...
            <h2>Sorry, No Supplier to display at the moment.</h2>
          </div>
          ) 
        : <div />}
